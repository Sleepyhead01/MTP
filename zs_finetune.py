...
MODEL = "gpt-4o"
OUTPUT_JSONL = f"{MODEL}_zeroshot.jsonl"
...
def process_entries():
    ...
    for entry in tqdm(entries, desc="Generating final responses"):
        try:
            ...
            prompt_text = """Analyze the image and explain the humor (if any) along with the underlying message/concept it represents. In other words, analyze the image and explain the cultural, scientific, or technical references and how they contribute to the joke or punchline (if any)."""  # Your prompt
            url = entry['image_url']
            if DEBUG_MODE:
                print("\n=== API REQUEST ===")
                print("url: ", url)
